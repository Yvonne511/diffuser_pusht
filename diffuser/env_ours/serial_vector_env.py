...
class SerialVectorEnv:
    ...
    def sample_random_init_goal_states(self, seed, fix_goal=False):
        init_state, goal_state = zip(*(self.envs[i].sample_random_init_goal_states(seed[i], fix_goal=fix_goal) for i in range(self.num_envs)))
    ...
    def prepare(self, seed, init_state, stabilize=False):
        ...
        for i in range(self.num_envs):
            ...
            o, s = env.prepare(cur_seed, cur_init_state, stabilize=stabilize)
            obs.append(o)
            state.append(s)
