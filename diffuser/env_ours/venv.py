    ...
    try:
        while True:
            ...
            if cmd == "step":
                ...
                obs, state_dct = env.prepare(data[0], data[1], data[2])
                if obs_bufs is not None:
                    _encode_obs(obs, obs_bufs)
                    obs = None
                p.send((obs, state_dct))
            elif cmd == "sample_random_init_goal_states":
                state = env.sample_random_init_goal_states(data[0], data[1])
                p.send(state)
            elif cmd == "eval_state":
...
class SubprocEnvWorker(EnvWorker):
    ...
    def prepare(self, seed, init_state, stabilize=False):
        self.parent_remote.send(["prepare", (seed, init_state, stabilize)])
        return self.parent_remote.recv()
    
    def sample_random_init_goal_states(self, seed, fix_goal=False):
        self.parent_remote.send(["sample_random_init_goal_states", (seed, fix_goal)])
        return self.parent_remote.recv()
...
class BaseVectorEnv(object):
    ...
    def prepare(self, seeds, init_states, stabilize=False):
        ...
        for i in range(self.env_num):
            o, s = self.workers[i].prepare(seeds[i], init_states[i], stabilize)
            obs_list.append(o)
            state.append(s)
    ...
    def sample_random_init_goal_states(self, seed, fix_goal=False):
        self._assert_is_not_closed()
        init_state, goal_state = zip(*(self.workers[i].sample_random_init_goal_states(seed[i], fix_goal=fix_goal) for i in range(self.env_num)))
