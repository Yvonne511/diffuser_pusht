...
class WallEnvWrapper(DotWall):
    def __init__(self, rng=42, wall_config=DEFAULT_CFG, fix_wall=True, cross_wall=False, fix_wall_location=32, fix_door_location=10, device='cpu', state_based=False, **kwargs):
        super().__init__(rng, wall_config, fix_wall, cross_wall, fix_wall_location=fix_wall_location, fix_door_location=fix_door_location, device=device, state_based=state_based, **kwargs)
    ...
    def prepare(self, seed, init_state, stabilize=False):
        """
        Reset with controlled init_state, stabilize option is ignored here
        """
        self.seed(seed)
